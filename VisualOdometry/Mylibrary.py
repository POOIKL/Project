...
def SaveFig(g, dg, num, CurPos, lenDict, savepath):
    ...
    # my_x_ticks = np.arange(0, 60, 20)
    # my_y_ticks = np.arange(0, 75, 20)
    ...
    nx.draw(g, nx.get_node_attributes(g, 'pos'), node_color=color_map, with_labels=True, node_size=400, alpha=1, font_size=15)
...
def direction_detect(new_frame, prev_gray, prev, FP, LP, color):
    ...
    for _, (new, old) in enumerate(zip(good_new, good_old)):
        ...
        # 画光流
        cv2.line(new_frame, (a, b), (c, d), color, 2)
        cv2.circle(new_frame, (a, b), 2, color, -1)
